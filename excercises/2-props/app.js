...
var emailType = (props, propName, componentName) => {
  warning(
    //use props[propName] so it doesn't matter if prop is called email or userId or whatever
...
var sizeType = (props, propName, componentName) => {
  warning(
    ...
    `Invalid size '${props[propName]}'. Check the render method of '${componentName}'.`
  );
};

var Gravatar = React.createClass({
  //like setting specific type on element
  propTypes: {
    email: emailType,
    size: sizeType
  },
//like setting defaults in backbone
  getDefaultProps () {
    return {
      size: 16
    };
...
React.render(<App userObj={USERS}/>, document.body);
